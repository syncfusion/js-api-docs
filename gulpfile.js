...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/js-api-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/js-api-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
             ...
             if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../js-api-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/js-api-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../js-api-docs/` + changedFileName, `../../../gitlabRepo/js-api-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/js-api-docs/')) {
                            shelljs.cp('-rf', `../js-api-docs/` + changedFileName, `../../../gitlabRepo/js-api-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/js-api-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/js-api-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/js-api-docs`);
